...
For example, ratings on \newjp{ESG} factors provide information about the sustainability performance of a company or a financial instrument, by assessing its exposure to sustainability risks and/or its impact on people and the environment \parencite{europeancommission2024a}.
...
Among those, the work in \parencite{lo2021} introduces a general framework for quantifying \newjp{the} impact of impact investing.
...
It is a common assumption to suppose that both the performance metric and the impact factor are identically distributed throughout time \newjp{and we will hold this assumption in this work.}
...
Formally: suppose that we have a dataset consisting of \newjp{\iid} realizations of the investment universe $\left\{\left(X_1, \Theta_1\right), \dots, \left(X_N, \Theta_N\right)\right\}$, the order statistics of the impact factor $\Xbf$ is defined as a random permutation $\sigma$ of its elements satisfying: 
...
\newjp{Recall that throughout the paper, we suppose that the realizations $X_1,\dots, X_N$ are \iid}.
...
An \textit{impact portfolio strategy}, as defined in \parencite{lo2021}, is a strategy that consider\newjp{s} both the performance metric $\Thetabf$ and the impact factor $\Xbf$.
...
In our impact investing framework, the problem is defined as finding a \newjp{generator} function $\mathcal{G}$ associated to a source of randomness $\Zbf$ such that: 
...
The problem of the existence of \newjp{tractable} models achieving \newjp{exactly} \eqref{eq:gen_learning_pb} is an open question. 
...
\begin{theorem}
  Let $\ve > 0$. There exists a family of \newjp{ReLU Neural Networks} $G_{N - r:N}^{NN}$ for $1 \leq r \leq N$ such that, using the sampling procedure of \Cref{alg:sampling_schucany} and setting
  \begin{equation}
...
\begin{theorem}
  ...
  With the sampling strategy of \Cref{alg:sampling_sukhatme}, there exist \newjp{ReLU Neural Networks} $F_{NN}$ and $F_{NN}^{-1}$ such that the error in approximating the uniform order statistics satisfies, in expectation,
  \begin{equation}
    \E|U_{r:N} - \hat U_{r:N}| \leq \ve\left[1 +  2\sum_{i=1}^r \frac{1}{N - \newjp{i} + 1}\right],
  \end{equation}
  with at most
  \begin{equation}
...
\begin{equation}
  \ve\left[1 +  2\sum_{i=1}^{\newjp{N}} \frac{1}{N - \newjp{i} + 1}\right] \sim \ve\ln(N),
\end{equation}
...
Regarding architecture, we consider both networks to be simple \newjp{MLP models}.
...
We provide more details on \newjp{hyperparameters in \Cref{sec:hyperparameters}.}
...
For this purpose, we choose a classic MLP architecture for the networks, illustrate that it can fit a classical benchmark dataset properly, and then assess visually up to which dimension \newjp{the model can} generate proper order statistics.
...
Once the best model is \newjp{selected}, we enhance it with an auxiliary regularization term that proxies the quality of the order-statistics property of the produced data points.
...
Precise definitions and background on the metrics are detailed in \newjp{\Cref{sec:metrics}}.
...
We \newjp{henceforth choose WGAN} as the best performing model and candidate for enhancement in the latter.
...
\begin{equation}
  ...
  \mathcal{L}_{\text{OSP}}(G) = \mathbb{E}_{z \sim p(z)} \left[ \frac{1}{d-1} \sum_{j=1}^{d-1} \left( \newjp{\max}\left\{ G(z)_j - G(z)_{j+1} + \varepsilon,\, 0 \right\} \right)^2 \right]
\end{equation}
...
% Chapter appendix - reset section counter for appendix numbering
\setcounter{section}{0}
\renewcommand{\thesection}{\thechapter.\Alph{section}}

\section{Additional Figures}\label{sec:addl-figs}
...
\section{Some theory of concomitants}
\subsection{Representation results for order statistics} \label{subsec:representation_OS}\label{sec:proof_representationos}
...
\section{Some elements on Functional approximation with Neural Network}
...
\section{Proofs}
\subsection{Proof of \Cref{prop:sampling_schucany}}\label{sec:proof_schucany}
...
\end{cases}
\]
Note that the triangular function can be approximated by a \newjp{ReLU} Neural Network with three neurons, see \parencite{allouche2025}.
...
For this sake, \newjp{consider $\ve \in [0, 1]$} and introduce the function :
...
\section{Experiments}
\subsection{More background on GANs}
